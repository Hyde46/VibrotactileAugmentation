...
public abstract class FingerModel : MonoBehaviour {
  ...
  */
  ...
  public Vector3 GetJointPosition(int joint) {
    ...
    if (joint >= NUM_BONES) {
        return GetTipPosition() + offsetVector;
    }
    if (controller_ != null && finger_ != null) {
      Vector3 local_position = finger_.Bone ((Bone.BoneType)(joint)).PrevJoint.ToUnityScaled (mirror_z_axis_);
      return controller_.transform.TransformPoint(local_position) + offset_ + offsetVector;
    }
    if (joints [joint]) {
        return joints[joint].position + offsetVector;
    }
  ...
  public Vector3 GetBoneCenter(int bone_type) {
    ...
    if (controller_ != null && finger_ != null) {
      ...
      return controller_.transform.TransformPoint(bone.Center.ToUnityScaled(mirror_z_axis_)) + offset_ + offsetVector;
    }
    if (bones [bone_type]) {
        return bones[bone_type].position + offsetVector;
    }
    return Vector3.zero + offsetVector;
  }
